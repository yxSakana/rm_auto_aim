...
SerialDriverNode::SerialDriverNode(const rclcpp::NodeOptions& options)
    : Node("serial_port", options),
      m_io_ctx(new IoContext(3)),
      m_driver(new drivers::serial_driver::SerialDriver(*m_io_ctx)) {
    initParamenters();
    RCLCPP_INFO(this->get_logger(), "%s",m_device_name.c_str());
    // Publisher
    m_receive_pub = this->create_publisher<
        custom_serial_interfaces::msg::Receive>("/custom_serial/receive", rclcpp::SensorDataQoS());
    // Serivce
    using SendPackage = custom_serial_interfaces::srv::SendPackage;
    m_send_service = this->create_service<SendPackage>("custom_serial/send", 
    ...
    try {
        ...
        if (!m_driver->port()->is_open()) m_driver->port()->open();
...
void SerialDriverNode::initParamenters() {
    ...
    if (fc_str == "none") {
        ...
        RCLCPP_ERROR(this->get_logger(), "%s ;<Usage> none OR hardware OR sofware", fc_str.c_str());
    }
    ...
    if (pt_str == "none") {
        ...
        RCLCPP_ERROR(this->get_logger(), "%s ;<Usage> none OR odd OR even", pt_str.c_str());
    }
    ...
    if (sb_str == "1") {
        ...
        RCLCPP_ERROR(this->get_logger(), "%s; <Usage> 1 OR 1.5 OR 2", sb_str.c_str());
    }

    m_port_cfg = std::make_unique<
        drivers::serial_driver::SerialPortConfig>(baud_rate, fc, pt, sb);
}
...
void SerialDriverNode::receiveData() {
    ...
    while (rclcpp::ok()) {
        ...
        if (buff[0] == 0x5A) {
            ...
            RCLCPP_WARN_STREAM(this->get_logger(), fmt::format("Error header: {:04X}", buff[0]));
        }
    }
}

void SerialDriverNode::sendData(uint8_t func_code, uint16_t id, uint16_t len, const uint8_t* data) {
    ...
    *(reinterpret_cast<uint16_t*>(pbuff + 4)) = len;
    ...
    auto crc = custom_serial::Verify_CRC16_Check_Sum(const_cast<uint8_t*>(data), len);
