...
void HikCameraNode::core() {
    ...
    while (rclcpp::ok()) {
        auto s = this->now();
        status = MV_CC_GetImageBuffer(handle, &out_frame, 1000);

        // RCLCPP_INFO(this->get_logger(), "Get Buffer: %f", (this->now() - s).seconds() * 1000);
        s = this->now();
        if (status == MV_OK) {
            // hik sdk
            ...
            // RCLCPP_INFO(this->get_logger(), "resize: %f", (this->now() - s).seconds() * 1000);
            s = this->now();
            // Pub
            ...
            // RCLCPP_INFO(this->get_logger(), "Free Buffer: %f", (this->now() - s).seconds() * 1000);
