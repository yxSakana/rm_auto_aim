...
//                 item.region.points(p.data());
//                 // auto rp = sortFeaturePoint({p, p+4});
//                 auto rp = sortFeaturePoint(p);
//                 for (int i = 0; i < 4; i++) {
//                     auto color = item.state == FanState::Target? cv::Scalar(0, 0, 255): cv::Scalar(0, 255, 255);
//                     cv::line(img, rp[i], rp[(i + 1) % 4], color, 2);
//                     cv::putText(img, std::to_string(i), rp[i], 0, 0.8, color, 2);
//                 }
//                 // drawRotatedRect(img, item.region, 3, item.state == FanState::Target? cv::Scalar(0, 0, 255): cv::Scalar(0, 255, 255));
...
//                     // item.state == FanState::Target? cv::Scalar(0, 0, 255): cv::Scalar(0, 255, 255));
//             }
//         }
//     }
//     // Debug publish
...
//     // Create fans base on current condition
...
//     std::vector<cv::Point2f> src(ps.begin(), ps.end());
//     std::sort(src.begin(), src.end(), [this](const cv::Point2f& a, const cv::Point2f& b) -> bool {
//         return cv::norm(a - m_r_box->center) < cv::norm(b - m_r_box->center);
//     });
//     for (int i = 0; i < len; i++) {
//         if (ps[i] == src[0]) {
//             begin = ps[(i + 1) % len] != src[1]? i: i + 1;
//             break;
//         }
//     }
//     for (int i = begin; i < begin + len; ++i)
//         result.push_back(ps[i % len]);
//     return result;
// }
